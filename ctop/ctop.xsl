...
<xsl:template mode="c2p" match="mml:csymbol/text()">
 <mml:mi><xsl:value-of select="."/></mml:mi><!-- Robin Green r.d.greenATlancaster.ac.uk, Christoph Lange langecATweb.de-->
</xsl:template>
...
<xsl:template mode="c2p" match="mml:lambda
				...
				|mml:bind[*[1][self::mml:csymbol='lambda']]"><!--dpc-->
 <mml:mrow>
	...
	<mml:mtr>
	  ...
	  <xsl:choose><!--dpc-->
	    <xsl:when  test="self::mml:piece">
	      <mml:mtd columnalign="left"><mml:mtext>&#160; if &#160;</mml:mtext></mml:mtd>
	      <mml:mtd><xsl:apply-templates mode="c2p" select="*[2]"/></mml:mtd>
	    </xsl:when>
	    <xsl:otherwise>
	      <mml:mtd colspan="2" columnalign="left"><mml:mtext>&#160; otherwise</mml:mtext></mml:mtd>
	    </xsl:otherwise>
	  </xsl:choose>
	</mml:mtr>
      </xsl:for-each>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:times]]
				|mml:apply[*[1][self::mml:csymbol='times']]"
	      name="times">
  <xsl:param name="p" select="0"/>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:int]]
 ...
 <mml:msubsup>
  <mml:mi>&#8747;<!--int--></mml:mi>
 <mml:mrow><xsl:apply-templates mode="c2p" select="mml:lowlimit/*|mml:interval/*[1]|mml:condition/*|mml:domainofapplication/*"/></mml:mrow>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:partialdiff]]" priority="1">
  <mml:mfrac>
	...
	<xsl:when test="not(mml:bvar/mml:degree) and not(mml:bvar[2])">
	  <mml:mo>&#8706;<!-- partial --></mml:mo>
	</xsl:when>
	<xsl:otherwise>
	  <mml:msup><mml:mo>&#8706;<!-- partial --></mml:mo>
	  <mml:mrow>
	    <xsl:choose>
		...
		<xsl:if test="count(mml:bvar[not(mml:degree)])&gt;0">
	      ...
	      <xsl:otherwise>
		<mml:mn><xsl:value-of select="number(sum(mml:bvar/mml:degree))+count(mml:bvar[not(mml:degree)])"/></mml:mn>
	      </xsl:otherwise>
	    </xsl:choose>
	  </mml:mrow>
	  </mml:msup>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:sum]]
                       |mml:apply[*[1][self::mml:csymbol='sum']]">
 <mml:mrow>
 <mml:msubsup>
  <mml:mo>&#8721;<!--sum--></mml:mo>
 <mml:mrow><xsl:apply-templates mode="c2p" select="mml:lowlimit|mml:interval/*[1]|mml:condition/*"/></mml:mrow><!-- Alexey Shamrin shamrinATmail.ru -->
 <mml:mrow><xsl:apply-templates mode="c2p" select="mml:uplimit/*|mml:interval/*[2]"/></mml:mrow>
 </mml:msubsup>
 <xsl:apply-templates mode="c2p" select="*[last()]"/>
</mml:mrow>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:product]]
                       |mml:apply[*[1][self::mml:csymbol='product']]">
 <mml:mrow>
 <mml:msubsup>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:limit]]
                       |mml:apply[*[1][self::mml:csymbol='limit']]">
 <mml:mrow>
 <mml:munder>
  <mml:mi>lim</mml:mi> <!-- Alexey Shamrin shamrinATmail.ru -->
...
<!-- 4.4.7.4 tendsto -->
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:tendsto]]|mml:reln[*[1][self::mml:tendsto]]">
 <xsl:param name="p"/>
<xsl:call-template name="binary">
 <xsl:with-param name="this-p" select="2"/>
 <xsl:with-param name="p" select="$p"/>
...
<xsl:template mode="c2p" match="
 mml:sin | mml:cos | mml:tan | mml:sec |
 mml:csc | mml:cot | mml:sinh | mml:cosh |
 mml:tanh | mml:sech | mml:csch | mml:coth |
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:scalarproduct]]
                       |mml:apply[*[1][self::mml:csymbol='scalarproduct']]">
<xsl:param name="p" select="0"/>
<xsl:call-template name="infix">
 <xsl:with-param name="this-p" select="2"/>
 <xsl:with-param name="p" select="$p"/>
...
<xsl:template name="set" >
  ...
  <mml:mrow>
   ...
   <mml:mrow><xsl:apply-templates mode="c2p" select="mml:bvar/*[not(self::mml:bvar or self::mml:condition)]"/></mml:mrow><!--dpc-->
   <mml:mo>|</mml:mo>
   <mml:mrow><xsl:apply-templates mode="c2p" select="mml:condition/node()"/></mml:mrow>
   </xsl:when>
   <xsl:otherwise>
    <xsl:for-each select="*[not(position()=1 and parent::mml:apply)]"><!--dpc-->
      <xsl:apply-templates mode="c2p" select="."/>
      <xsl:if test="position() !=last()"><mml:mo>,</mml:mo></xsl:if>
    </xsl:for-each>
   </xsl:otherwise>
