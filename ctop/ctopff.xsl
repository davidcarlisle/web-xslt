...
<x:template match="m:apply[*[1][self::m:rem]]
  ...
  <x:param name="p" select="0"/>
<x:call-template name="binary">
  <x:with-param name="mo"><mo>mod</mo></x:with-param>
  <x:with-param name="p" select="$p"/>
  <x:with-param name="this-p" select="3"/>
</x:call-template>
</x:template>
...
<x:template match="m:apply[*[1][self::m:times]]
  ...
  <x:for-each select="*[position()&gt;1]">
   <x:if test="position() &gt; 1">
    <mo>
    <x:choose>
      <x:when test="self::m:cn">&#215;<!-- times --></x:when>
      <x:otherwise>&#8290;<!--invisible times--></x:otherwise>
    </x:choose>
    </mo>
   </x:if> 
...
<x:template match="m:apply[*[1][self::m:diff]]" priority="1">
 ...
 <mrow><msup><mi>d</mi><x:apply-templates select="m:bvar/m:degree/node()"/></msup>
     <x:apply-templates  select="*[last()]"/></mrow>
 <mrow><mi>d</mi><msup><x:apply-templates
 select="m:bvar/node()"/><x:apply-templates
 select="m:bvar/m:degree/node()"/></msup>
</mrow>
</x:when>
<x:otherwise>
 <mrow><mi>d</mi><x:apply-templates select="*[last()]"/></mrow>
 <mrow><mi>d</mi><x:apply-templates select="m:bvar"/></mrow>
</x:otherwise>
 </x:choose>
 </mfrac>
</x:template>
